...
- 資料庫連線設定在 `backend/src/main/resources/application.properties`,帳號 `sa`、密碼 `1234`、port `1433`。
  SQL Server 的安裝設定雷點整理在 `backend/src/main/resources/init/README.md`
- 資料庫是拋棄式的——每次啟動由 `DatabaseResetter` 整庫重置並載入示範資料。
  想暫時保留手動輸入的資料,把 `app.db.reset` 改為 `false`
...
1.  **架構缺陷**：將圖片存入資料庫會嚴重拖垮效能，且資料庫僅在本機執行，缺乏實際上線部署。
2.  **商業邏輯單純**：僅有基礎功能與按讚，技術深度稍嫌單薄。

### 三、 學生的進階課題

教學範例僅此而已，若學生希望將其轉化為高含金量的面試作品，後續的優化將是他們自己的課題。這包含但不限於：
*   **資料庫與儲存**：部署正式的資料庫伺服器,並導入雲端物件儲存處理圖片,不再把圖片存進資料表。
*   **實務架構**：前後端分離部署上線、串接第三方登入、導入 Redis 效能優化。
*   **工程品質**：撰寫自動化測試與 CI/CD 流程。
*   **查詢效能（N+1 問題）**：貼文列表對每一筆資料都各自查詢發文者、按讚數與按讚狀態，
    一頁 10 筆就是 30 條以上的 SQL（打開 `spring.jpa.show-sql` 即可觀察）。
    可研究 `@EntityGraph` 預載關聯、以 `IN` 一次撈回整頁資料再於記憶體彙總等優化手法。
*   **分頁策略**：目前的 offset 分頁在「瀏覽途中有新貼文」時會出現內容位移／重複，
    可研究 cursor-based pagination（以「最後一筆的 id / 時間」為錨點）解決。
